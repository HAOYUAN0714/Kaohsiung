...
xhr.onload = function(){
  ...
  //頁面更新函式
  let zoneContent = []; // 要計算個別區域有多少必資料用的
  let selectZone = '';
  let hotzone = '';
  const hotzones = ['苓雅區','三民區','岡山區','前鎮區'];
  function showInfo(e){
    hotzone = e.target.textContent; // 如果是點擊熱門區的話，取這個值
    selectZone = e.target.value; // 選擇區，取這個值
    console.log(e);
    zoneContent = [];
    for(var i = 0;i < data.result.records.length; i++){
      //判斷 如果'選擇'的值等於zone 或 '點選'的內容等於zone 執行innerHTML更新
      if(selectZone == data.result.records[i].Zone || hotzone == data.result.records[i].Zone){
  ...
  function updatePages (e) {
    ...
    if (e.target.dataset.to && e.target.nodeName === "A") { // 點擊上下頁
      ...
      for (let i = 0; i <  [...pagination.children].length; i++) {
        let child = [...pagination.children];
        if([...child[i].classList].indexOf('active') !== -1) {
          currentPage = i;
        }
      }
      currentPage = currentPage + Number(e.target.dataset.to); // 索引從 pre 開始算所以不用 + 1
      ...
      console.log('currentPage',currentPage,'totalPage',totalPage,'currentPage < totalPage' , currentPage < totalPage);
